...
class DirectedWeightedGraphAlgorithmsImplTest {
    ...
    @Test
    void init() {
        boazBenMoshe.init(dg);
        assertEquals(boazBenMoshe.getGraph().getNode(1).getInfo(),"boaz");
    }
    ...
    void getGraph() {
        ...
        for (int i = 0; i < 16; i++) {
            List<NodeData> a = boazBenMoshe.shortestPath(i, i + 1);
            assertEquals(a.get(1).getKey(), boazBenMoshe.getGraph().getNode(i + 1).getKey());
        }
    }
...
//        System.out.println(d.getNode(0).getKey()+" "+dg.getNode(0).getKey());
    ...
    void isConnected() {
        ...
        boazBenMoshe.load("data/G1.json");
        assertTrue(boazBenMoshe.isConnected());
    }
    ...
    void shortestPathDist() {
        ...
        assertEquals(le, 0);
        boazBenMoshe.load("data/G1.json");
        assertEquals(boazBenMoshe.shortestPathDist(0, 16), 1.3118716362419698);
        assertEquals(boazBenMoshe.shortestPathDist(5, 6), 1.734311926030133);
        assertEquals(boazBenMoshe.shortestPathDist(9, 11), 1.2861739185896588 + 1.4962204797190428);
    ...
    void center() {
        boazBenMoshe.load("data/G1.json");
        assertEquals(boazBenMoshe.center().getKey(), 8);
    ...
    void save() {
        DirectedWeightedGraphAlgorithmsImpl dtest = new DirectedWeightedGraphAlgorithmsImpl(dg);
        boazBenMoshe.load("data/G1.json");
        boazBenMoshe.save("data/ElizebthIzkovich.json");
        dtest.load("data/ElizebthIzkovich.json");
        for (int i = 0; i < 17; i++) {
            assertEquals(boazBenMoshe.getGraph().getNode(i).toString(), dtest.getGraph().getNode(i).toString());
        }
    }
    ...
    void load() {
        boazBenMoshe.load("data/G1.json");
        for (int i = 0; i < 17; i++) {
            assertEquals(boazBenMoshe.getGraph().getNode(i).getKey(), i);
        }
        assertEquals(boazBenMoshe.getGraph().getEdge(15, 16).getWeight(), 1.8726071511162605);
    }
